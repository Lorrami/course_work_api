...
namespace TestAPIIgnat.Extensions
{
        ...
        public static T ToClass<T>(this Dictionary<string, AttributeValue> dict)
        {
            ...
            foreach (var kv in dict)
            {
                ...
                if (property != null)
                {
                    ...
                    else if (!string.IsNullOrEmpty(kv.Value.L.ToString()))
                    {
                        var list1 = kv.Value.L;
                        var listOfString = new List<string>();
                        foreach (var value in list1)
                        {
                            listOfString.Add(Convert.ToString(value));
                        }
                        property.SetValue(obj, listOfString);
                    }
                }
            }
            return (T)obj;
